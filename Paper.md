...
- APeS： 这代表着极小尺寸目标（Extra Small），通常是指非常小的目标，可能在图像中只占据很少的像素空间。

- APrS： 表示相对小尺寸目标（Relative Small），这些目标比较小但相对于整个图像来说稍大一些。

- APgS： 对应着相对大尺寸目标（Relative Large），指的是相对于图像而言比较大的目标。
